...
class ChooseStartSecurityFragment : BaseFragment() {
    ...
    private fun setupButton() {
        securityButton.setOnClickListener {
            startActivity(PrivateKeyStoreSupportActivity.createIntent(securityButton.context))
            finish()
        }
        homeButton.setOnClickListener {
            if (!PinCodeHelper.isAvailable(homeButton.context)) {
    ...
    private fun showSecurityWarningDialog() {
        activity?.let {
            ...
            RaccoonSelectDialog.createDialog(
                    ...
                    .show(it.supportFragmentManager, RaccoonSelectDialog::class.java.toString())
        }
    }
    ...
    private val getMessage by lazy {
        getString(arguments?.getInt(KEY_MESSAGE) ?: R.string.app_name)
    }
